...
Route::get('articles', function(Request $request){
    ...
    // $perPage = $request->input('per_page', 2); // 요청에서 'per_page'파라미터를 받아옴. 없으면 기본값 2로 설정
    // $skip = ($page - 1) * $perPage; // 몇 개의 항목을 건너뛰는지

    // Article 모델에서 데이터 조회
    // Eager Loading 첫 번째 방법 : with()
    // with('user') : 연관된 'user'관계 데이터를 Eager Loading으로 미리 로드
